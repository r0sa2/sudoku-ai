    ...
    def __init__(self, grid: list[list[int]]) -> None:
        ...
        for r in range(1, 10):
            for c in range(1, 10):
                ...
                # value
                self.clauses.append([self._get_var_num(r, c, v) for v in range(1, 10)])
                # Create clauses to ensure that each cell does not contain two
                # distinct values
                for v in range(1, 10):
                    for vp in range(v + 1, 10):
                        self.clauses.append(
                            [-self._get_var_num(r, c, v), -self._get_var_num(r, c, vp)]
                        )
        # Create clauses to ensure that each row (column) contains distinct
        # values
        for i in range(1, 10):
            self.clauses.extend(
                self._get_distinct_value_clauses([(i, j) for j in range(1, 10)])
            )
            self.clauses.extend(
                self._get_distinct_value_clauses([(j, i) for j in range(1, 10)])
            )
        ...
        for r in range(1, 10):
            for c in range(1, 10):
                if grid[r - 1][c - 1] != 0:
                    self.clauses.append([self._get_var_num(r, c, grid[r - 1][c - 1])])
