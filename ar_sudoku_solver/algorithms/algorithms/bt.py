...
class SudokuBT:
    ...
    It is assumed that a solution exists.
    """
        ...
        for r, c in product(range(9), range(9)):
            if self.grid[r][c] != 0:
                self.encodings.update(
                    self._get_encodings(r, c, self.grid[r][c])
                )

    def _get_encodings(self, r: int, c: int, n: int) -> set[str]:
        """
        Returns a set of row, column, and box encodings for the case wherein
        the no. `n` is added to the grid at the cell with row `r` and column
        `c`.
        """
    ...
    def _backtrack(self, r: int, c: int) -> bool:
        ...
        and top-to-bottom order starting at the cell with row `r` and column
        `c`.
        """
        if r == 9 and c == 0:  # Sudoku is solved
            ...
            return (
                self._backtrack(r, c + 1)
                if c < 8
                else self._backtrack(r + 1, 0)
            )
        else:
            for n in range(1, 10):
                encodings = self._get_encodings(r, c, n)
